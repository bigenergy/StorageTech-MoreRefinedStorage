...
public class ItemDiskCustom
extends Item
implements IItemRegister,
IStorageDiskProvider,
IRenderItemRegister {
    public ItemDiskCustom() {
        this.setRegistryName("customdisk");
        this.setTranslationKey("customdisk");
        ...
        if (!world.isRemote) {
			...
			*/
            if (!stack.hasTagCompound()) {
                ...
                API.instance().getStorageDiskManager(world).set(id, API.instance().createDefaultItemDisk(world, this.getCapacity(stack)));
    ...
    public ItemRenderCast[] getItemsRender() {
        return new ItemRenderCast[]{new ItemRenderCast(0, "customdisk")};
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ItemStack itemStack;
        if (!worldIn.isRemote && playerIn.isSneaking() && (itemStack = playerIn.getHeldItem(handIn)) != null && itemStack.getItem() instanceof ItemDiskCustom) {
